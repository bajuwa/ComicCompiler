...
def _find_breakpoint(page, image):
    ...
    batch_sample_size = args.break_points_increment * args.break_points_multiplier
    return imgmag.find_breakpoint(image, max(0, offset), batch_sample_size, args.break_points_increment,
                                  args.split_on_colour, args.colour_error_tolerance, args.colour_standard_deviation)
...
def _define_page(page, images):
    ...
    for image in images:
        ...
        if args.breakpoint_detection_mode == 1:
            ...
            breakpoint_row = _find_breakpoint(page, image)
            if breakpoint_row >= 0:
                page.crop_from_bottom = image.height - breakpoint_row
                return
        else:
            if imgmag.ends_in_breakpoint(image, args.split_on_colour, args.colour_error_tolerance,
...
def _stitch_page(page):
    ...
    imgmag.combine_vertically(_get_image_paths(page.images), args.output_directory + page.name)
